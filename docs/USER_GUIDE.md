...
Use `Run Research Workflow` from a candidate page for the default assisted path. The workflow finds or reuses publications, builds or reuses a broader researcher profile, ranks papers, skips unsuitable papers and papers without lawful full text, selects the best available paper, retrieves the PDF, extracts text, runs Gemini-backed analysis through the provider interface, verifies the official email, verifies attachments, generates a local draft, and stops for review. The candidate page shows the current workflow stage, researcher-intelligence themes and clusters, selected paper, PDF status, failures, and retry action. The selected paper page shows DOI, OpenAlex ID, authorship position, selection score, score reasons, lawful PDF source, SHA-256 hash, extraction status, retrieval result, and alternatives considered.

Use `Choose a different paper` when you want to override the automatic selection.

The manual review page is the final page for this phase. It shows the verified recipient, source URL, subject, body, word count, selected paper, summary, evidence map, attachment checklist, warnings, and sentence-level support checks. The copy buttons use the browser clipboard only. You must paste into Outlook and attach the resume and portfolio yourself.
